...
function payIncomeTax(
  ...
  // log(`taxDue for ${source} on ${startOfTaxYear} = ${showObj(taxDue)}, totalTaxDue = ${totalTaxDue}, alreadyPaid = ${alreadyPaid}`);
...
interface Liabilities {
  inTaxYear: {
    ...
    incomeTax: {
      ...
      fixedIncome: undefined | Map<string, number>;
      flexibleIncome: undefined | Map<string, LiabilityTotalAndSources>; 
    },
    NI: {
      total: undefined | Map<string, LiabilityTotalAndSources>;
      fixedIncomeNi: undefined | Map<string, number>;
      flexibleIncomeNi: undefined | Map<string, number>;
    },
  };
  inTaxMonth: {
    incomeTax: {
      total: undefined | Map<string, number>; 
      fixedIncome: undefined | Map<string, number>;
      flexibleIncomeTM: undefined | Map<string, LiabilityTotalAndSources>; 
    },
    NI: {
      total: undefined | Map<string, number>;
      fixedIncome: undefined | Map<string, number>;
      flexibleIncome: undefined | Map<string, number>;
      ...
      if (liability === person) {
        ...
        if (cashVal === undefined) {
          ...
          const flexibleIncomesMap =
            incomes.inTaxYear.incomeTax.flexibleIncome;
          ...
          if (personVal === undefined) {
            personVal = { amount: 0, sources: [] };
            flexibleIncomesMap.set(person, personVal);
          }
          const sourceKey = `auto-transfer from ${valueKey}`;
          addToTaxLiability(personVal, amountToTransfer, sourceKey,
            CASH_ASSET_NAME);
  ...
  for (const taxType of taxTypes) {
    ...
    if (taxType === incomeTax && personLiabilityMap !== undefined) {
      ...
      const incomeFixedPersonLiabilityMap =
        incomes.inTaxYear.incomeTax.fixedIncome;
      ...
      for (const [person, amount] of incomeTaxPersonLiabilityMap) {
        ...
        /* eslint-disable-line no-restricted-syntax */
        // log(`go to pay income tax for ${person}, amount = ${showObj(amount)} for ${date}`);
        // log(`alreadyPaid = ${alreadyPaid}, amountFixed = ${amountFixed}, alreadyPaidForFixed = ${alreadyPaidForFixed}`);
        ...
        flexibleIncomesMap.set(person, { amount: 0, sources: [] });

        const fixedIncomesMap = incomes.inTaxYear.incomeTax.fixedIncome;
        if (fixedIncomesMap === undefined) {
          throw new Error("fixedIncomesMap should be defined");
        }
        fixedIncomesMap.set(person, 0);

        const liableIncomeTaxInTaxMonth = incomes.inTaxMonth.incomeTax.total;
        ...
        const liableFixedIncomeTaxInTaxMonth =
          incomes.inTaxMonth.incomeTax.fixedIncome;
      ...
      const niFixedPersonLiabilityMap = incomes.inTaxYear.NI.fixedIncomeNi;
      if (niFixedPersonLiabilityMap === undefined) {
        throw new Error("expect niFixedPersonLiabilityMap to have been set up");
      }

      let liableIncomeNIInTaxMonth = incomes.inTaxMonth.NI.total;
      /* istanbul ignore if  */ //redundant untested
      if (liableIncomeNIInTaxMonth === undefined) {
        log("Error : expect maps to have been set up in accumulateLiability");
        liableIncomeNIInTaxMonth = new Map<string, number>();
        incomes.inTaxMonth.NI.total = liableIncomeNIInTaxMonth;
      }

      let liableFixedIncomeNIInTaxMonth = incomes.inTaxMonth.NI.fixedIncome;
      /* istanbul ignore if  */ //redundant untested
      if (liableFixedIncomeNIInTaxMonth === undefined) {
        log("Error : expect maps to have been set up in accumulateLiability");
        liableFixedIncomeNIInTaxMonth = new Map<string, number>();
        incomes.inTaxMonth.NI.fixedIncome = liableFixedIncomeNIInTaxMonth;
      }
      ...
      for (const [person, amount] of niPersonLiabilityMap) {
        const niFromFixedIncomeMap = incomes.inTaxYear.NI.fixedIncomeNi;
...
function payTaxEstimate(
  ...
  let liableIncomeTaxInTaxMonth = liabilities.inTaxMonth.incomeTax.total;
  if (liableIncomeTaxInTaxMonth === undefined) {
    liableIncomeTaxInTaxMonth = new Map<string, number>();
    liabilities.inTaxMonth.incomeTax.total = liableIncomeTaxInTaxMonth;
  }

  let liableFixedIncomeTaxInTaxMonth =
    liabilities.inTaxMonth.incomeTax.fixedIncome;
  if (liableFixedIncomeTaxInTaxMonth === undefined) {
  ...
  let liableFlexibleIncomeTaxInTaxMonth =
    liabilities.inTaxMonth.incomeTax.flexibleIncomeTM;
  if (liableFlexibleIncomeTaxInTaxMonth === undefined) {
    liableFlexibleIncomeTaxInTaxMonth = new Map<string, LiabilityTotalAndSources>();
    liabilities.inTaxMonth.incomeTax.flexibleIncomeTM =
  ...
  for (const [person, liableIncome] of liableIncomeTaxInTaxMonth) {
    ...
    const flexIncomesInTaxMonth: {
      amount: number,
      sourceDescription: string,
    }[] = [];
    for(const s of flexValObj.sources){
      const existing = flexIncomesInTaxMonth.find((f)=>f.sourceDescription === s.source);
      if(existing === undefined){
        flexIncomesInTaxMonth.push({
          amount: s.amount,
    ...
    if (monthOfTaxYear === 3) {
      ...
      const fixedEstimateForAnnualIncome = fixedIncomeSoFar 
          + fixedVal * numMonthsLeftInTaxYear;

      let flexIncomeSoFar = liabilities.inTaxYear.incomeTax.flexibleIncome?.get(person);
      if (flexIncomeSoFar === undefined) {
        flexIncomeSoFar = {
          amount: 0,
          sources: [],
        };
      ...
      let estimatesForAnnualFlexIncome:{
        amount: number,
        sourceDescription: string,
      }[] = [];
      for(const source of flexIncomesInTaxMonth){
        const val = flexIncomeSoFar.amount
            + source.amount * numMonthsLeftInTaxYear;
        estimatesForAnnualFlexIncome.push({
          amount: val,
          sourceDescription: source.sourceDescription,
        });
      }
      // log(`estimatesForAnnualFlexIncome = ${showObj(estimatesForAnnualFlexIncome)}`);

      const estimateAnnualTaxDueFixed: {
        amountLiable: number;
        rate: number;
      }[] = calculateIncomeTaxPayable(
        fixedEstimateForAnnualIncome,
        startYearOfTaxYear,
        values,
      );
      const annualEstimateFixed = sumTaxDue(estimateAnnualTaxDueFixed);

      let paidUpIncome = fixedEstimateForAnnualIncome;

      let payableAnnualAmountsFlex: {
        amount: number,
        sourceDescription: string,
      }[] = [];

      for (const flexIncome of estimatesForAnnualFlexIncome){
        const estimateAnnualTaxDueFlex: {
          amountLiable: number;
          rate: number;
        }[] = calculateIncrementalIncomeTaxPayable(
          paidUpIncome,
          flexIncome.amount,
          startYearOfTaxYear,
          values,
        );
        const annualEstimateFlex = sumTaxDue(estimateAnnualTaxDueFlex);
        payableAnnualAmountsFlex.push({
          amount: annualEstimateFlex,
          sourceDescription: flexIncome.sourceDescription,
        })
        paidUpIncome += flexIncome.amount;
      }

      // log(`payableAnnualAmountsFlex = ${showObj(payableAnnualAmountsFlex)}`);

      let estimateMonthTaxDueFixed = 0.0;
      let estimateMonthTaxDueFlex = 0.0;
      ...
      let annualEstimateSoFar = annualEstimateFixed;

      const annualEstimateDueFlex: {
        amount: number,
        sourceDescription: string,
      }[] = [];
      
      for(const annualAmount of payableAnnualAmountsFlex){
        annualEstimateDueFlex.push({
          amount: annualEstimateSoFar + annualAmount.amount - taxPaidSoFar,
          sourceDescription: annualAmount.sourceDescription,
        });
        annualEstimateSoFar += annualAmount.amount;
      }

      // log(`es incomeTax for ${startYearOfTaxYear}, annualEstimateDueFlex is ${showObj(annualEstimateDueFlex)}`);
      ...
      if (estimateMonthTaxDueFixed > 0) {
        ...
        estimatesPaid += estimateMonthTaxDueFixed;
      ...
      for(const flexEstimate of annualEstimateDueFlex){
        estimateMonthTaxDueFlex =
          Math.floor((flexEstimate.amount / (numMonthsLeftInTaxYear + 1)) * 100 + 0.00001) / 100;

        // log(`from flexEstimate.amount ${flexEstimate.amount} we should pay this month ${estimateMonthTaxDueFlex}`);
        if (estimateMonthTaxDueFlex > 0) {
          ...
          for(const s of flexValObj.sources){
            if(s.source === flexEstimate.sourceDescription){

              const matchedTarget = targets.find((t)=>t.targetDescription === s.target);
              if(matchedTarget === undefined){
                ...
                matchedTarget.amount += s.amount;
              }
              totalTargetAmount += s.amount;
            }
          }
          for(const t of targets){
            const proportion = t.amount / totalTargetAmount;

            let taxPaidFrom = CASH_ASSET_NAME;
            if (flexEstimate.sourceDescription.startsWith(crystallizedPension)){
              let assetValue = getNumberValue(values, t.targetDescription, false);
              if(assetValue !== undefined && assetValue >= estimateMonthTaxDueFlex * proportion){
                taxPaidFrom = t.targetDescription;
              }
            }
            // log(`adjust target ${taxPaidFrom} for tax estimate ${estimateMonthTaxDueFlex * proportion}`);
            adjustAsset(
              -estimateMonthTaxDueFlex * proportion,
              getTaxMonthDate(startYearOfTaxYear, monthOfTaxYear),
              values,
              growths,
              evaluations,
          ...
          estimatesPaid += estimateMonthTaxDueFlex;
          incomeTaxMonthlyPaymentsPaid.set(person, estimatesPaid);
        }
      }
    }
    liableIncomeTaxInTaxMonth.set(person, 0);
    liableFixedIncomeTaxInTaxMonth.set(person, 0);
    liableFlexibleIncomeTaxInTaxMonth.set(person, { amount: 0, sources: [] });
  }
}
function payNIEstimate(
  ...
  // NI
  let liableNIInTaxMonth = incomes.inTaxMonth.NI.total;
  if (liableNIInTaxMonth === undefined) {
    liableNIInTaxMonth = new Map<string, number>();
    incomes.inTaxMonth.NI.total = liableNIInTaxMonth;
  }
  let liableNIInTaxMonthFromFixed = incomes.inTaxMonth.NI.fixedIncome;
  if (liableNIInTaxMonthFromFixed === undefined) {
    liableNIInTaxMonthFromFixed = new Map<string, number>();
    incomes.inTaxMonth.NI.fixedIncome = liableNIInTaxMonthFromFixed;
  }
  let nIMonthlyPaymentsPaid = taxMonthlyPaymentsPaid.NIxTotal;
  if (nIMonthlyPaymentsPaid === undefined) {
    nIMonthlyPaymentsPaid = new Map<string, number>();
...
function addToTaxLiability(
  ...
  if (incomeValue !== 0.0) {
    ...
    const matchedSource = taxLiability.sources.find((s) => {
      return s.source === sourceDescription && s.target === targetDescription;
    });
    if (matchedSource) {
  ...
  */
  ...
  if (type === incomeTax) {
    ...
    if (personLiabilityMap === undefined) {
      ...
      >();
      incomes.inTaxYear.incomeTax.fixedIncome = new Map<string, number>();
      incomes.inTaxYear.incomeTax.flexibleIncome = new Map<string, LiabilityTotalAndSources>();
      personLiabilityMap = incomes.inTaxYear.incomeTax.total;
    }
    ...
    if (personLiabilityMap === undefined) {
      ...
      incomes.inTaxYear.NI.fixedIncomeNi = new Map<string, number>();
      incomes.inTaxYear.NI.flexibleIncomeNi = new Map<string, number>();
      personLiabilityMap = incomes.inTaxYear.NI.total;
    }
  }
  if (personLiabilityMap !== undefined) {
    ...
    addToTaxLiability(taxLiability, incomeValue, sourceDescription, targetDescription);
  }
  if (type === incomeTax) {
    if (isFlexibleIncome) {
      const flexibleIncomesMap = incomes.inTaxYear.incomeTax.flexibleIncome;
      if (flexibleIncomesMap !== undefined) {
        ...
        if (personVal === undefined) {
          ...
          flexibleIncomesMap.set(liability, personVal);
        }
        const sourceKey = sourceDescription;
        addToTaxLiability(personVal, incomeValue, sourceKey, targetDescription);
      }
    }
    if (isFixedIncome) {
      const fixedIncomesMap = incomes.inTaxYear.incomeTax.fixedIncome;
    ...
    let liableIncomeTaxInTaxMonth = incomes.inTaxMonth.incomeTax.total;
    /* istanbul ignore if  */ //redundant untested
    if (liableIncomeTaxInTaxMonth === undefined) {
      liableIncomeTaxInTaxMonth = new Map<string, number>();
      incomes.inTaxMonth.incomeTax.total = liableIncomeTaxInTaxMonth;
    }
    let liableFixedIncomeTaxInTaxMonth =
      incomes.inTaxMonth.incomeTax.fixedIncome;
    /* istanbul ignore if  */ //redundant untested
    ...
    if (isFlexibleIncome) {
      ...
      if (taxFlexibleLiability === undefined) {
        taxFlexibleLiability = { amount: 0, sources: []  };
        liableFlexibleIncomeTaxInTaxMonth.set(liability, taxFlexibleLiability);
      }
      const sourceKey = sourceDescription;
      addToTaxLiability(taxFlexibleLiability, incomeValue, sourceKey, targetDescription);
    }
  ...
  if (type === nationalInsurance) {
    ...
    let liableNIFromFixedInTaxMonth = incomes.inTaxMonth.NI.fixedIncome;
    /* istanbul ignore if */
    if (liableNIInTaxMonth === undefined) {
      log(`Error: don't expect liableNIInTaxMonth to be undefined!`);
      liableNIFromFixedInTaxMonth = new Map<string, number>();
      incomes.inTaxMonth.NI.fixedIncome = liableNIFromFixedInTaxMonth;
    }
...
function handleCGTLiability(
  ...
  if (t.TO_ABSOLUTE) {
    ...
    addToTaxLiability(currentcgtVal, gain, `${t.NAME}`, CASH_ASSET_NAME);
    cgtMap.set(cgtLiability, currentcgtVal);
    ...
    addToTaxLiability(currentcgtVal, proportionGain, `${t.NAME}`, CASH_ASSET_NAME);
    // if (fromWord.includes('ESPP') || fromWord.includes('RSU')) {
...
function handleTaxObligations(
  ...
  // TODO we want to say we're at the start of a new tax month on 1 April 
  // as well as the start of a new tax year on 6 April
...
function getEvaluationsInternal(
  ...
  // the person who is liable to pay and
  // a value for the accrued liable value as a tax month progresses
  const incomes: Liabilities = {
    inTaxMonth: {
      incomeTax: {
        total: undefined,
        fixedIncome: undefined,
        flexibleIncomeTM: undefined,
      },
      NI: {
        total: undefined,
        fixedIncome: undefined,
        flexibleIncome: undefined,
      },
    },
    inTaxYear: {
      cgt: {
      ...
      NI: {
        total: undefined,
        fixedIncomeNi: undefined,
        flexibleIncomeNi: undefined,
      }
    },
  };
