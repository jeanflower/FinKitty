...
import { getNumberAndWordParts, makeStringFromCashValue } from '../../stringUtils';
...
export class SettingFormatter extends React.Component<SimpleFormatterProps, {}> {
  public render() {
    ...
    if(nwp.wordPart === 'USD' && nwp.numberPart !== undefined){
      tidyValue = `${makeStringFromCashValue(nwp.numberPart.toFixed(2), '')}USD`;
    }
    ...
    return (
      <OverlayTrigger
        ...
        overlay={(props: any) => (
          <Tooltip
            {...props}
          >{`${this.props.name}:${tidyValue}`}</Tooltip>
        )}
      >
        <span>{tidyValue}</span>
      </OverlayTrigger>
