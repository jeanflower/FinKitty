...
describe(' chart data tests', () => {
  ...
  it('delta chart data for cpi assets', (done) => {
    ...
    expectEvals(evals, 8, 'savings', 'Sat Sep 01 2018', 577.10, 2);
    ...
    expectEvals(evals, 12, 'savings', 'Tue Jan 01 2019', 620.00, 2);
    ...
    expect(result.assetData[1].item.NAME).toBe('growth/savings');
    {
    const chartPts = result.assetData[1].chartDataPoints;
    expect(chartPts.length).toBe(16);
    expectChartData(chartPts, 0, '37', 0, -1);
    ...
    expect(result.assetData[2].item.NAME).toBe('sell some/savings');
    {
