import { getVariableDateExampleData } from "../../models/exampleModels";
import { ChartData } from "../../types/interfaces";
import { log, printDebug, showObj } from "../../utils/utils";
import { calcOptimizer } from "../../views/tablePages";
import { printTestCodeForEvals, printTestCodeForChart, getTestEvaluations, expectEvals } from "./algoTestUtils";
...
describe('optimizer tests', () => {
  it('should load optimizer model', (done) => {
    ...
    const cd: ChartData = calcOptimizer(model);
    // log(`cd === \n${showObj(cd)}`);

    expect(JSON.stringify(cd)).toEqual(JSON.stringify(
      {
        "labels": [
            1,
            2,
