...
export function checkTransaction(t: DbTransaction, model: DbModelData): string {
  ...
  if (t.TYPE === payOffDebt) {
    ...
    if (!recognised) {
      return `payoff debt type of transaction ${t.NAME} not a recognised format`;
    }
  }
  ...
  if (t.TYPE === revalueDebt) {
    ...
    if (!recognised) {
      return `revalue debt type of transaction ${t.NAME} not a recognised format`;
    }
  }
  if (t.TYPE === revalueInc) {
    ...
    if (!recognised) {
      return `revalue income type of transaction ${t.NAME} not a recognised format`;
    }
  }
  if (t.TYPE === revalueExp) {
    ...
    if (!recognised) {
      return `revalue expense type of transaction ${t.NAME} not a recognised format`;
    }
  }
  if (t.TYPE === custom) {
    // there are a known set of type of
